...
// iptables -t nat -N SKY
// iptables -t nat -A SKY -p tcp -j REDIRECT --to-port 8080
// iptables -A INPUT -p tcp -m string --string 'SKYcoming' --algo bm -m recent --set --name SKY --rsource -j ACCEPT
// iptables -A INPUT -p tcp -m string --string 'SKYleaving' --algo bm -m recent --name SKY --remove -j ACCEPT
// iptables -t nat -A PREROUTING -p tcp --dst 192.168.1.18 --dport 80 --syn -m recent --rcheck --seconds 3600 --name SKY --rsource -j SKY

// iptables -t nat -D PREROUTING -p tcp --dst 192.168.1.18 --dport 80 --syn -m recent --rcheck --seconds 3600 --name SKY --rsource -j SKY
// iptables -D INPUT -p tcp -m string --string 'SKYleaving' --algo bm -m recent --name SKY --remove -j ACCEPT
// iptables -D INPUT -p tcp -m string --string 'SKYcoming' --algo bm -m recent --set --name SKY --rsource -j ACCEPT
// iptables -t nat -F SKY
// iptables -t nat -X SKY

const CHAIN_NAME = "SKY"
const START_FORWARDING = "skycoming"
const STOP_FORWARDING = "skyleaving"
